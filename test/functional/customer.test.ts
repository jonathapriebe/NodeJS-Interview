...
describe('Customer functional tests', () => {
  ...
  describe('When find a Customer', () => {
    it('Should return a customer by name with success', async () => {
      const { body, status } = await global.testRequest.get('/customer?name=João');
      const customer = {
        name: 'João',
        gender: 'Masculino',
        age: 15,
    ...
    it('Should return 404 if not find a customer', async () => {
      const { body, status } = await global.testRequest.get('/customer?name=Pedro');
  ...
  describe('When create a Customer', () => {
    it('Should create a customer with success', async () => {
      ...
      const { body, status } = await global.testRequest.post('/customer').send(newCustomer);
    ...
    it('should return 404 when City not found', async () => {
      ...
      const response = await global.testRequest.post('/customer').send(newCustomer);
    ...
    it('should return 422 when there is a validation error', async () => {
      ...
      expect(response.body).toEqual({
        error: 'Customer validation failed: dt_birthday: Path `dt_birthday` is required.',
      });
    });
  ...
  describe('When update a Customer', () => {
    it('Should update a customer with success', async () => {
      ...
      const { body, status } = await global.testRequest.put('/customer/60782456831a012733ff97bd').send({name: 'Joãozinho'});
      expect(status).toBe(200);
      expect(body).toEqual(expect.objectContaining(customer));
    });

    it('should return 422 when name in body not found', async () => {
      const response = await global.testRequest.put('/customer/60782456831a012733ff97bd').send();
      expect(response.status).toBe(422);
      expect(response.body).toEqual({
    ...
    it('should return 404 when Customer not found', async () => {
      const response = await global.testRequest.put('/customer/60782456831a012733ff9744').send({name: 'Joãozinho'});
      expect(response.status).toBe(404);
      expect(response.body).toEqual({
  ...
  describe('When delete a Customer', () => {
    it('Should delete a customer with success', async () => {
      const { body, status } = await global.testRequest.delete('/customer/60782456831a012733ff97bd');
      expect(status).toBe(200);
      expect(body).toEqual({message:'Customer deleted'});
    });

    it('should return 404 when Customer not found', async () => {
      const response = await global.testRequest.delete('/customer/60782456831a012733ff9744');
      expect(response.status).toBe(404);
      expect(response.body).toEqual({
        error: 'Customer not found.'
      });
